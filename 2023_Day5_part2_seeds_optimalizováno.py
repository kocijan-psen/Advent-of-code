...
for level in type_:
    processed_garten[level] = sorted(
        [(item[0], item[1], item[2]) for item in garten[level]],
        key=lambda x: x[0]
    )

# Hlavní logika s optimalizací
for i, s in enumerate(seeds_part2):
    for level in type_:
        intervals = processed_garten[level]
        
        # Vyhledání pomocí bisect
        idx = bisect_left(intervals, (s,))
        
        # Zkontrolujeme, zda je s v rozsahu
        if idx < len(intervals) and intervals[idx][0] <= s < intervals[idx][1]:
            s += intervals[idx][2]
            seeds_part2[i] = s  # Aktualizace hodnoty
            break  # Ukončení cyklu, přejít na další s

print ("asdfasdf")
# print("final: ", final_seed)
final_ = sorted(seeds_part2)
